...
BEGIN
  ...
  -- Create auth user with a new UUID
  ...
  -- Create profile with the same UUID as auth user
...
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT uid, auth_password FROM university_credentials
  LOOP
    PERFORM create_university_user(r.uid, r.auth_password);
  END LOOP;
END $$;